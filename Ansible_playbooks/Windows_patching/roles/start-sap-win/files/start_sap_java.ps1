$services = get-wmiobject win32_service |where-object{($_.Name -like '*SAP???_??*') -and ($_.pathname -like "*\j*") -and $_.state -eq "Running"}|Select-object Name,displayname,state,pathname
