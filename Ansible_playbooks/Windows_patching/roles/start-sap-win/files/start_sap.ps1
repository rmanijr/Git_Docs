$services = get-wmiobject win32_service |where-object{($_.Name -like '*SAP???_??*') -and ($_.pathname -notlike '*SCS*') -and ($_.pathname -notlike "*\j*") -and $_.state -eq "Running"}|Select-object Name,displayname,state,pathname
